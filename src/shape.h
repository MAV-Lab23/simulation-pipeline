...
void SetCircleCells(uchar* out_cells, const Vec2i& pos, int rad) {
	...
	for (int j = 0; j < GRID_SIZE.y; j++)
	{
		...
		{
			Vec2i dist{ i - grid_pos.x, j - grid_pos.y };
			out_cells[i + offset] = (int)(dist.x * dist.x + dist.y * dist.y <= rad2);
		}
	}
}

bool RectanglePointOverlap(const Vec2i& coord, const Vec2i& pos, const Vec2i& size, float angle /* degrees */) {
	Vec2i t = { pos.x - coord.x, pos.y - coord.y };

	Vec2i half_size = { size.x / 2, size.y / 2 };
...
void SetRectangleCells(uchar* out_cells, const Vec2i& pos, const Vec2i& size, float angle /* degrees */) {
	const Vec2i grid_pos = WorldToGrid(pos);
	for (int j = 0; j < GRID_SIZE.y; j++)
	{
		int offset = j * GRID_SIZE.x;
